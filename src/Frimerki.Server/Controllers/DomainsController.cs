...
public class DomainsController(IDomainService domainService, ILogger<DomainsController> logger)
    : ControllerBase {
    /// <summary>
    ...
    public async Task<ActionResult<DomainListResponse>> GetDomains() {
        try {
            ...
            var domains = await domainService.GetDomainsAsync(userRole, userDomainId);
            return Ok(domains);
        } catch (Exception ex) {
            logger.LogError(ex, "Error retrieving domains");
    ...
    public async Task<ActionResult<DomainResponse>> GetDomain(string domainName) {
        try {
            var domain = await domainService.GetDomainByNameAsync(domainName);
            return Ok(domain);
        } catch (ArgumentException ex) {
            return NotFound(new { error = ex.Message });
        } catch (Exception ex) {
            logger.LogError(ex, "Error retrieving domain {DomainName}", domainName);
    ...
    public async Task<ActionResult<DomainResponse>> CreateDomain([FromBody] DomainRequest request) {
        try {
            ...
            var domain = await domainService.CreateDomainAsync(request);

            logger.LogInformation("Domain '{DomainName}' created successfully", request.Name);
            ...
            logger.LogError(ex, "Error creating domain {DomainName}", request.Name);
    ...
    public async Task<ActionResult<DomainResponse>> UpdateDomain(string domainName, [FromBody] DomainUpdateRequest request) {
        try {
            ...
            // TODO: Check if user is HostAdmin
            var domain = await domainService.UpdateDomainAsync(domainName, request);

            logger.LogInformation("Domain '{DomainName}' updated successfully", domainName);
            return Ok(domain);
        } catch (ArgumentException ex) {
            return NotFound(new { error = ex.Message });
        } catch (Exception ex) {
            logger.LogError(ex, "Error updating domain {DomainName}", domainName);
    ...
    public async Task<ActionResult<DomainResponse>> PatchDomain(string domainName, [FromBody] DomainUpdateRequest request) {
        try {
            ...
            logger.LogInformation("Domain '{DomainName}' patched successfully", domainName);
            return Ok(domain);
        } catch (ArgumentException ex) {
            return NotFound(new { error = ex.Message });
        } catch (Exception ex) {
            logger.LogError(ex, "Error patching domain {DomainName}", domainName);
    ...
    public async Task<IActionResult> DeleteDomain(string domainName) {
        try {
            // TODO: Check if user is HostAdmin
            await domainService.DeleteDomainAsync(domainName);

            logger.LogInformation("Domain '{DomainName}' deleted successfully", domainName);
            return NoContent();
        } catch (ArgumentException ex) {
            return NotFound(new { error = ex.Message });
        } catch (InvalidOperationException ex) {
            ...
            logger.LogError(ex, "Error deleting domain {DomainName}", domainName);
    ...
    public async Task<ActionResult<DkimKeyResponse>> GetDkimKey(string domainName) {
        try {
            var dkimKey = await domainService.GetDkimKeyAsync(domainName);
            return Ok(dkimKey);
        } catch (ArgumentException ex) {
            return NotFound(new { error = ex.Message });
        } catch (Exception ex) {
            logger.LogError(ex, "Error retrieving DKIM key for domain {DomainName}", domainName);
    ...
    public async Task<ActionResult<DkimKeyResponse>> GenerateDkimKey(string domainName, [FromBody] GenerateDkimKeyRequest request) {
        try {
            ...
            var dkimKey = await domainService.GenerateDkimKeyAsync(domainName, request);

            logger.LogInformation("DKIM key generated for domain '{DomainName}' with selector '{Selector}'",
            ...
            logger.LogError(ex, "Error generating DKIM key for domain {DomainName}", domainName);
