...
public class MockUserServiceForEmailController : IUserService {
    ...
    public async Task<PaginatedInfo<UserResponse>> GetUsersAsync(int skip = 1, int take = 50,
        string domainFilter = null) =>
        new() { Items = [], TotalCount = 0 };

    public async Task<UserResponse> CreateUserAsync(CreateUserRequest request) => new();

    public async Task<UserResponse> UpdateUserAsync(string email, UserUpdateRequest request) => null;

    public async Task<bool> UpdateUserPasswordAsync(string email, UserPasswordUpdateRequest request) => false;

    public async Task<bool> DeleteUserAsync(string email) => false;

    public async Task<UserStatsResponse> GetUserStatsAsync(string email) => new();

    public async Task<bool> UserExistsAsync(string email) => false;

    public async Task<UserResponse> AuthenticateUserAsync(string email, string password) => null;

    public async Task<Frimerki.Models.Entities.User> AuthenticateUserEntityAsync(string email, string password) => null;

    public async Task<Frimerki.Models.Entities.User> GetUserEntityByEmailAsync(string email) => null;

    public async Task<bool> ValidateEmailFormatAsync(string email) => true;
